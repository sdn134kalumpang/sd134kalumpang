// modules/e-dokumen/features/arsip.js - FIXED - No regex error - Base64 Firestore - Taat v3
// Path: sd134kalumpang/modules/e-dokumen/features/arsip.js - hapus tes.js lama di image_2274b6.png
// Firestore: schools/40312947/e_dokumen_arsip
...
window.init_arsip = function(container){
  ...
  + '<div style="display:flex;gap:8px;"><button id="btnExportArsip" style="background:white;color:#0d3b66;padding:8px 16px;border-radius:20px;font-size:12px;font-weight:700;">📊 Export XLS</button><button id="btnTambahArsip" style="background:#ffcc00;color:#0d3b66;padding:8px 16px;border-radius:20px;font-size:12px;font-weight:700;">+ Tambah</button></div>'
  + '</div>'
  ...
  + '<div style="display:flex;gap:8px;"><button id="btnSimpanArsip" style="flex:1;background:#0d3b66;color:white;padding:10px;border-radius:8px;font-size:12px;font-weight:700;">Simpan Firestore</button><button id="btnBatalArsip" style="background:#f1f5f9;padding:10px 16px;border-radius:8px;font-size:12px;">Batal</button></div>'
  ...
  function loadData(){
    ...
    if(window.FirebaseService && FirebaseService.isEnabled()){
      FirebaseService.getAll('e_dokumen_arsip').then(function(list){
  ...
  document.getElementById('btnSimpanArsip').onclick = function(){
    ...
    if(editId){
      if(window.FirebaseService && FirebaseService.isEnabled()){
        FirebaseService.update('e_dokumen_arsip', editId, obj).then(function(){ document.getElementById('btnBatalArsip').click(); loadData(); });
      }
    } else {
      if(window.FirebaseService && FirebaseService.isEnabled()){
        FirebaseService.add('e_dokumen_arsip', obj).then(function(){ document.getElementById('btnBatalArsip').click(); loadData(); });
      }
    }
  };
  ...
  window.hapusArsip = function(fid){
    if(!confirm('Hapus permanen Firestore?')) return;
    if(window.FirebaseService && FirebaseService.isEnabled()){
      FirebaseService.delete('e_dokumen_arsip', fid, fid).then(function(){ loadData(); });
    }
  };
