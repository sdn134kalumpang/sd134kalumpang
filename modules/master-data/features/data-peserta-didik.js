...
// Update: 🔥 FIXED - Sinkronisasi Firestore + Prevent Listener Memory Leak
...
window.init_data_peserta_didik = window.init_data_peserta_didik || function(container){
  ...
  <div class="welcome-banner" style="background:linear-gradient(135deg,#0f172a,#1e3a8a);">
    <div> <h1 style="color:white;">🎓 Data Peserta Didik - Master Data</h1> <p style="color:rgba(255,255,255,0.7);">Auto-fill: ${auto.nama_guru} • Kepsek: ${auto.nama_kepsek} • Kop: ${md.kop?.nama_sekolah || school.nama}</p> </div>
    ...
    list.forEach(s=>{
      ...
      tbody.innerHTML += `<tr style="border-bottom:1px solid #f1f5f9;"> 
        ...
        <td style="padding:8px;"><div style="display:flex;gap:4px;">
          <button onclick="editSiswa('${s.id}')" style="font-size:10px;background:#f1f5f9;padding:4px 8px;border-radius:6px;">✏️</button>
          <button onclick="hapusSiswa('${s.id}')" style="font-size:10px;background:#fef2f2;color:#dc2626;padding:4px 8px;border-radius:6px;">🗑️</button>
        </div></td> 
      </tr>`;
    });
  ...
  // ========================================================================
  // 🔥 PERBAIKAN 1: LISTENER FIRESTORE HANYA DIPANGGIL 1 KALI SAAT INIT
  // ========================================================================
  if(window.FirebaseService && FirebaseService.isEnabled()){
    ...
    FirebaseService.getPesertaDidik().then(list=>{
      if(list && list.length){
        ...
        let mdSync = ServiceMenu.getMasterData();
        mdSync.peserta_didik = list;
        ServiceMenu.saveMasterData(mdSync);
        renderTable(ServiceMenu.isAdmin() ? siswaList : ServiceMenu.filterOwnerOnly(siswaList));
      }
    });
    // 2. Realtime listener (HANYA SEKALI, tidak di dalam tombol!)
  ...
  // ===== IMPORT & TEMPLATE LOGIC (TETAP UTUH) =====
  ...
  function downloadTemplateXLSX(){
    ...
    const instruksi = [
      ...
      ['NPSN', school.npsn], ['Sekolah', school.nama], ['Alamat', school.alamat],
      ['Tahun Ajaran', school.tahunAjaran], ['Dibuat oleh', auto.nama_guru], ['NIP', auto.nip_guru], [''],
      ['Kolom Wajib: NIS,Nama,Kelas,JK'], ['JK: L atau P'], ['Kelas: 1-6'], ['Jangan hapus header baris pertama']
    ];
  ...
  document.getElementById('btnProsesImport').onclick = ()=>{
    ...
    if(file.name.endsWith('.xlsx') || file.name.endsWith('.xls')){
      alert('Untuk Excel .xlsx/.xls, silahkan Save As → CSV (Comma delimited) di Excel terlebih dahulu, lalu upload file CSV nya.');
      return;
    }
    ...
    reader.onload = function(e){
      ...
      try{
        ...
        const idx = {
          nis: headers.indexOf('nis'), nisn: headers.indexOf('nisn'), nama: headers.indexOf('nama'),
          kelas: headers.indexOf('kelas'), jk: headers.indexOf('jk'), ttl: headers.indexOf('ttl'),
          alamat: headers.indexOf('alamat'), ayah: headers.indexOf('ayah'), ibu: headers.indexOf('ibu')
        };
        if(idx.nama===-1){ alert('Header Nama tidak ditemukan! Gunakan template resmi.'); return; }
        ...
        preview.innerHTML = `
          ...
          <button id="btnSimpanImport" style="margin-top:10px;background:#16a34a;color:white;padding:8px 16px;border-radius:8px;font-size:12px;font-weight:700;width:100%;">💾 Simpan ${parsedImportData.length} Data ke Master Data & Firestore</button>
        `;
        
        // ========================================================================
        // 🔥 PERBAIKAN 2: TAMBAHKAN PEMANGGILAN FIRESTORE SAAT SIMPAN IMPORT
        // ========================================================================
        document.getElementById('btnSimpanImport').onclick = async ()=>{
          ...
          // 🔥 KIRIM KE FIRESTORE (Batch)
          if(window.FirebaseService && FirebaseService.isEnabled()){
            await FirebaseService.addBatchPesertaDidik(parsedImportData);
          }

          preview.innerHTML = `<div style="padding:10px;background:#dcfce7;border-radius:8px;font-size:12px;">✅ Berhasil import ${withOwner.length} siswa! Data sudah masuk Master Data dan Firestore.</div>`;
  ...
  // ========================================================================
  // 🔥 PERBAIKAN 3: HAPUS & EDIT MENGGUNAKAN firestore_id
  // ========================================================================
  window.hapusSiswa = async function(id){
    if(!confirm('Hapus siswa ini?')) return;
    // Cari berdasarkan id lokal ATAU firestore_id
    const siswa = siswaList.find(s=> String(s.id) === String(id) || String(s.firestore_id) === String(id));
    if(!siswa) return;

    if(window.FirebaseService && FirebaseService.isEnabled() && (siswa.firestore_id || siswa.id)){
      await FirebaseService.deletePesertaDidik(siswa.firestore_id || siswa.id, siswa.id);
      // Biarkan listener Firestore yang otomatis mengupdate siswaList & renderTable (lebih aman & konsisten)
    } else {
      let md = ServiceMenu.getMasterData();
  ...
  window.editSiswa = function(id){
    // Cari berdasarkan id lokal ATAU firestore_id
    const s = siswaList.find(x=> String(x.id) === String(id) || String(x.firestore_id) === String(id));
    if(!s) return;
    ...
    document.getElementById('f_nama').dataset.editId = s.id; // Simpan ID lokal untuk referensi
    document.getElementById('btnSimpanSiswa').textContent = '💾 Update Siswa';
  };

  // ========================================================================
  // 🔥 PERBAIKAN 4: TAMBAHKAN PEMANGGILAN FIRESTORE SAAT SIMPAN MANUAL
  // ========================================================================
  document.getElementById('btnSimpanSiswa').onclick = async function(){
    ...
    if(editId){
      ...
      if(idx>=0) {
        ...
        // 🔥 Update ke Firestore
        if(window.FirebaseService && FirebaseService.isEnabled()){
          const fid = md.peserta_didik[idx].firestore_id || md.peserta_didik[idx].id;
          await FirebaseService.updatePesertaDidik(fid, obj);
        }
      }
      ...
      // MODE TAMBAH BARU
      const newSiswa = ServiceMenu.addOwner({ id: Date.now(), ...obj });
      md.peserta_didik.push(newSiswa);
