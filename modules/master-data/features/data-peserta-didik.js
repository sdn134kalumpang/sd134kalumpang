...
const meta = `# Template Import Peserta Didik - SDN 134 Kalumpang - Dibuat oleh: ${auto.nama_guru} - NIP: ${auto.nip_guru} - ${new Date().toLocaleDateString('id-ID',{day:'numeric',month:'long',year:'numeric'})}`;
function esc(v){
  if(v==null) return '';
  const s=String(v);
  if(s.includes(',') || s.includes('"') || s.includes('\n')){
...
const csv = [kop, meta, `# Format: NIS,NISN,Nama,Kelas(1-6),JK(L/P),TTL,Alamat,Ayah,Ibu`, '', csvBody].join('\n');
...
const headers = ['NIS','NISN','Nama Lengkap','Kelas','JK','TTL','Alamat','Ayah','Ibu','Status'];
const example = [
...
['Max 48 siswa sesuai SDN 134 Kalumpang']
];
let html = `<html><head><meta charset="UTF-8"><style>
  body{font-family:Arial;font-size:11px;}
  .kop{text-align:center;font-weight:bold;font-size:14px;margin-bottom:2px;}
  .subkop{text-align:center;font-size:11px;color:#475569;margin-bottom:2px;}
  th{background:#0d3b66;color:white;padding:8px;border:1px solid #000;}
  td{padding:6px;border:1px solid #000;}
  .info{font-size:10px;color:#64748b;margin:8px 0;}
  .yellow{background:#ffcc00;color:#0d3b66;}
</style></head><body>
  <div class="kop">${school.nama.toUpperCase()}</div>
  <div class="subkop">${school.alamat} - NPSN ${school.npsn} - Akreditasi ${school.akreditasi}</div>
  <div class="subkop">TEMPLATE IMPORT PESERTA DIDIK - Tahun Ajaran ${school.tahunAjaran}</div>
  <div class="subkop">Dibuat oleh: ${auto.nama_guru} - NIP: ${auto.nip_guru} - ${new Date().toLocaleDateString('id-ID',{day:'numeric',month:'long',year:'numeric'})}</div>
  <br>
  <table border="1" style="border-collapse:collapse;width:100%;"><tr><th colspan="2" style="background:#0d3b66;color:white;">INFORMASI SEKOLAH</th></tr>`;
instruksi.forEach(r=>{ html+= `<tr><td style="font-weight:bold;background:#f7f9fc;">${r[0]||''}</td><td>${r[1]||''}</td></tr>`; });
html+= `</table><br>`;
html+= `<table border="1" style="border-collapse:collapse;width:100%;"><tr>${headers.map(h=>`<th class="yellow">${h}</th>`).join('')}</tr>`;
example.forEach(r=>{ html+= `<tr>${r.map(c=>`<td>${c}</td>`).join('')}</tr>`; });
html+= `<tr><td style="color:#94a3b8;font-style:italic;">(isi sesuai data asli)</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>`;
html+= `</table><div class="info">Petunjuk: NIS unik, Kelas 1-6, JK L/P, TTL boleh pakai koma (sudah otomatis pakai kutip). Hapus baris contoh jika tidak perlu.</div></body></html>`;
const blob = new Blob([html], {type:'application/vnd.ms-excel;charset=utf-8;'});
const url = URL.createObjectURL(blob);
const a = document.createElement('a'); a.href=url; a.download=`Template_Import_Peserta_Didik_SDN134_${school.tahunAjaran.replace('/','-')}.xls`; a.click();
}
...
window.init_data_peserta_didik = window.init_data_peserta_didik;
