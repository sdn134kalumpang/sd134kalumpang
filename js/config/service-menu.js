...
// Perbaikan: Typo key, Update URL E-Portal, Auto target="_blank" untuk link eksternal
...
const PATH_CONFIG = {
  ...
  pages: {
    ...
    kop_administrasi: '/sd134kalumpang/modules/master-data/index.html?fitur=kop'
  }
};

const konfigurasiFitur = {
  'layanan-portal': [
    { nama: 'SIMPKB', icon: '🆔', link: 'https://portal.simpkb.id', isExternal: true },
    { nama: 'SINDARA', icon: '📊', link: 'https://sindara.gurudikdas.kemendikdasmen.go.id/pendaftaran/komunitas/login', isExternal: true },
    { nama: 'SIMACCA', icon: '🏫', link: '#', isExternal: true, status: 'soon' },
    { nama: 'Data Perpustakaan', icon: '📚', link: 'https://data.perpusnas.go.id/login', isExternal: true },
    { nama: 'Siaga', icon: '🕌', link: 'https://siagapendis.kemenag.go.id/login', isExternal: true },
    { nama: 'Info GTK', icon: '👨‍🏫', link: 'https://info.gtk.kemendikdasmen.go.id/', isExternal: true },
    { nama: 'E-Kinerja', icon: '💼', link: '#', isExternal: true, status: 'soon' },
    { nama: 'myASN', icon: '👤', link: 'https://myasn.bkn.go.id/', isExternal: true }
  ],
  'dokumen-arsip': [
...
const ALL_FEATURES = [
  ...
  { id: 'e_portal', label: 'E-Portal', icon: '🌐', sub: ['SIMPKB','SINDARA','SIMACCA','Data Perpustakaan','Siaga','Info GTK','E-Kinerja','myASN'], links: konfigurasiFitur['layanan-portal'] },
...
const ServiceMenu = {
  ...
  renderSidebar: function(containerId='dynamicSidebar'){
    ...
    const iconMap = {
      ...
      'SIMPKB':'🆔','SINDARA':'📊','SIMACCA':'','Data Perpustakaan':'📚','Siaga':'🕌','Info GTK':'👨🏫','E-Kinerja':'💼','myASN':'👤'
    };
    ...
    ALL_FEATURES.filter(f => f.id!=='dashboard' && f.id!=='pengaturan' && f.id!=='laporan').forEach(feat => {
      ...
      const subHtml = `<div class="sub-menu ${isLong?'sub-menu-long':''}" style="display:none;${isLong?'max-height:260px;overflow-y:auto;':''}">${allowedSubs.map(s => { 
        ...
        const isSoon = link === '#';
        const clickAttr = isSoon ? 'onclick="event.preventDefault(); alert(\'Fitur ini sedang dalam pengembangan (Coming Soon).\');"' : '';
        return `<a data-sub="${slug}" href="${link}" ${targetAttr} ${clickAttr}><span>${ic}</span> ${s}</a>`; 
      }).join('')}</div>`;
